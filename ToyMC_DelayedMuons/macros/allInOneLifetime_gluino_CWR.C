...
ExtraAxis anotherScale (const TH1* refHist, double scale, int color, const char* title, double offset) {
  ...
  axis->ImportAxisAttributes ((TAxis *)refHist->GetXaxis());
  axis->SetTitle(title);
...
ExtraAxis anotherScaleRight (const TH1* refHist, double scale, int color, const char* title, double offset) {
  ExtraAxis result;
  double x0 = refHist->GetXaxis()->GetXmin();
  double x1 = refHist->GetXaxis()->GetXmax();
  double y0 = refHist->GetMinimum();
  ...
  TGaxis* axis = new TGaxis(x1, y0, x1, y1, y0*scale,y1*scale,510,"+GS");
  axis->ImportAxisAttributes ((TAxis *)refHist->GetXaxis());
  axis->SetTitle(title);
  axis->SetTextColor (color);
  axis->SetLineColor (color);
...
void allInOneLifetime(double lumi=4560.) {
  ...
  //plots.calculateCrossSections(7,4,3,0,39,9); //400 GeV 
  //plots.calculateCrossSections(7,4,3,3,39,9); //1000 GeV 
  plots.calculateCrossSections(7,4,3,11,39,9); //2600 GeV                                                                                                         
  ...
  double mchamp2ref      = 2000;// gluino 1000GeV 2016  
...
//h->GetYaxis()->SetTitle("#sigma #times BF #times #varepsilon_{stop} #times #varepsilon_{det} [pb]  ");
  ...
  */
  ...
  // mchamp LIMIT
  //TLine *lmchamp = new TLine(7.5e-8, 94.8/mchamp2ref, 1e6, 94.8/mchamp2ref); //400 GeV
  TLine *lmchamp = new TLine(7.5e-8, 0.32/mchamp2ref, 1e6, 0.32/mchamp2ref); //1000 GeV
  //TLine *lmchamp = new TLine(7.5e-8, 0.0000503/mchamp2ref, 1e6, 0.0000503/mchamp2ref); //2600 GeV
  ...
  t1mchamp = new TLatex(0.001, 0.1/mchamp2ref, "#sigma_{theory} (m_{#tilde{g}} = 1000 GeV)");
...
/*
  ...
  */
  ...
  canvas->Print("allInOneLifetime_combinedgluino_1000GeV.pdf");
}
