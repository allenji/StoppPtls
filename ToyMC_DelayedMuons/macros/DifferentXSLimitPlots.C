...
DifferentXSLimitPlots::DifferentXSLimitPlots(double tpLumi) :
  tpLumi_(tpLumi)
{
...
DifferentXSLimitPlots::~DifferentXSLimitPlots() {

}

void DifferentXSLimitPlots::readToyFile(std::string filename) {
...
void DifferentXSLimitPlots::readTimeProfileFile(std::string filename) {
...
void DifferentXSLimitPlots::readLimitMchampData(std::string filename) {

  std::cout << "Reading limit mchamp data from " << filename << std::endl;
  ...
  while (file >> m >> er >> xst >> el >> b >> eb >> n >> cl >> exmean >> lo1sig >> hi1sig >> lo2sig >> hi2sig) {
    ...
    mchampEffLumi.push_back(el);
    mchampExpBG.push_back(b);
    mchampExpBGErr.push_back(eb);
    mchampObs.push_back(n);
    mchampObsLimit.push_back(cl);
    mchampExpLimit.push_back(exmean);
    mchampExpLimitLo1Sig.push_back(lo1sig);
    mchampExpLimitHi1Sig.push_back(hi1sig);
    mchampExpLimitLo2Sig.push_back(lo2sig);
    mchampExpLimitHi2Sig.push_back(hi2sig);
    
    // mchamp theory info
    if (xst > 0.) {
      mchampTheoryMass.push_back(m);
      mchampTheoryXS.push_back(xst);
    }
  ...
  // print out to check
  std::cout << "Read " << count << " mchamp mass points" << std::endl;
  for (unsigned c=0; c<count; ++c) {
    std::cout << mchampMass[c] <<" "<<mchampEffReco[c]<< " " << mchampEffLumi[c] << " " << mchampObsLimit[c] << " " << mchampExpLimit[c] << " " << mchampExpLimitLo1Sig[c] << " " << mchampExpLimitHi1Sig[c] << " " << mchampExpLimitLo2Sig[c] << " " << mchampExpLimitHi2Sig[c] << std::endl;
  }
...
void DifferentXSLimitPlots::readStauData(std::string filename) {
...
void DifferentXSLimitPlots::calculateCrossSections(unsigned gluinoIndex, unsigned stopIndex, unsigned mchampIndex, unsigned toyIndex, unsigned tpIndex) {
  ...
  for (int l=0; l<nToyLifetime; ++l) {
    ...
    xsProdExpLimitLo1SigToyMchamp.push_back( (toyExpLimit[l]-toyExpLimitLo1Sig[l]) / (toyEffLumi[l] * mchampEffReco[mchampIndex]) );
    xsProdExpLimitHi1SigToyMchamp.push_back( (toyExpLimitHi1Sig[l]-toyExpLimit[l]) / (toyEffLumi[l] * mchampEffReco[mchampIndex]) );
    xsProdExpLimitLo2SigToyMchamp.push_back( (toyExpLimit[l]-toyExpLimitLo2Sig[l]) / (toyEffLumi[l] * mchampEffReco[mchampIndex]) );
    xsProdExpLimitHi2SigToyMchamp.push_back( (toyExpLimitHi2Sig[l]-toyExpLimit[l]) / (toyEffLumi[l] * mchampEffReco[mchampIndex]) );
  ...
  */
  ...
  for (unsigned i=0; i<mchampMass.size(); ++i) {
		
    if (mchampEffReco[i] > 0.) {
      massMchamp.push_back(mchampMass[i]);

      //observed
      xsProdMassMchamp.push_back( mchampObsLimit[i] / (mchampEffLumi[i] * mchampEffReco[i]) );
      if (tpLifetime.size()>tpIndex)
			  
	xsProdMassTPMchamp.push_back( tpObsLimit[tpIndex] / (tpLumi_ * mchampEffReco[i]) );

      //expected mchamp
      xsProdExpMassMchamp.push_back( mchampExpLimit[i] / (mchampEffLumi[i] * mchampEffReco[i]) );
      xsProdExpLo1SigMassMchamp.push_back( (mchampExpLimit[i]-mchampExpLimitLo1Sig[i]) / (mchampEffLumi[i] * mchampEffReco[i]) );
      xsProdExpHi1SigMassMchamp.push_back( (mchampExpLimitHi1Sig[i]-mchampExpLimit[i]) / (mchampEffLumi[i] * mchampEffReco[i]) );
      xsProdExpLo2SigMassMchamp.push_back( (mchampExpLimit[i]-mchampExpLimitLo2Sig[i]) / (mchampEffLumi[i] * mchampEffReco[i]) );
      xsProdExpHi2SigMassMchamp.push_back( (mchampExpLimitHi2Sig[i]-mchampExpLimit[i]) / (mchampEffLumi[i] * mchampEffReco[i]) );

      std::cout << " mass=" << mchampMass.at(i) << " recoEff=" << mchampEffReco[i] << std::endl;
...
void DifferentXSLimitPlots::calculateIntercepts(){
  ...
  TGraph* m_theory       = getMchampTheory();
  //TGraph* m_expected     = getExpMassLimitMchamp();
  TGraphAsymmErrors* m_expected_asym= getExpMassLimitMchamp1Sig();
  TGraph* m_observed     = getMassLimitMchamp();
  TGraph* m           = getLimitMchamp();

  std::cout <<"Calculating Mchamp Mass Limits"<<std::endl;
  std::cout <<"\tCalculating Expected Limit"<<std::endl; 
  //double m_massLimit_exp= calculateMassLimits(m_theory, m_expected); 
  std::vector<double> m_MASS=calculateMassLimitsWithErrors(m_theory, m_expected_asym);
  double m_massLimit_exp=m_MASS[0];
  std::cout <<"\tCalculating Observed Limit"<<std::endl;
  double m_massLimit_obs= calculateMassLimits(m_theory, m_observed);
  std::cout <<"\tCalculating Limit"<<std::endl;
  double m_massLimit = calculateMassLimits(m_theory, m);
  std::cout <<"\n"<<std::endl;
  ...
  //dump out mchamp limits
  if (m_massLimit_exp>-1){
    std::cout <<"Mchamp mass limit - counting expt expected = " << m_massLimit_exp; 
    if (m_MASS[1]>0) std::cout<<"  "<<m_MASS[1]-m_massLimit_exp;
    else std::cout <<"  - N/A";
    if (m_MASS[2]>0) std::cout<<"  +"<<m_MASS[2]-m_massLimit_exp;
    else std::cout <<"  + N/A";
    std::cout <<std::endl;
  }
  else
    std::cout <<"Mchamp mass limit - counting expt expected:  Could not be determined!"<<std::endl;
  if (m_massLimit_obs>-1)
    std::cout <<"Mchamp mass limit - counting expt observed = " << m_massLimit_obs << endl;
  else
    std::cout <<"Mchamp mass limit - counting expt observed:  Could not be determined!"<<std::endl;
  if (m_massLimit>-1)
    std::cout <<"Mchamp mass limit - counting expt observed (NB) = " << m_massLimit << endl;
  else
    std::cout <<"Mchamp mass limit - counting expt observed (NB):  Could not be determined!"<<std::endl;
  std::cout <<"\n"<<std::endl;



} // void DifferentXSLimitPlots::calculateMassLimits()



double DifferentXSLimitPlots::calculateMassLimits(TGraph* gTheory, TGraph* gData) {
...
std::vector<double> DifferentXSLimitPlots::calculateMassLimitsWithErrors(TGraph* gTheory, TGraphAsymmErrors* gData) {
  /* 
...
TGraphAsymmErrors* DifferentXSLimitPlots::getExpLimitGluino2Sig() {
	return new TGraphAsymmErrors(xsProdExpLimitToyGluino.size(),
								&toyLifetime[0],
								&xsProdExpLimitToyGluino[0],
								0,
...
TGraphAsymmErrors* DifferentXSLimitPlots::getExpLimitStop2Sig() {
	return new TGraphAsymmErrors(xsProdExpLimitToyStop.size(),
								&toyLifetime[0],
								&xsProdExpLimitToyStop[0],
								0,
...
TGraph* DifferentXSLimitPlots::getLimitMchampBasic() {
	return new TGraph(xsLimitToyMchamp.size(), &toyLifetime[0], &xsLimitToyMchamp[0]);
}
TGraph* DifferentXSLimitPlots::getLimitMchamp() {
	return new TGraph(xsProdLimitToyMchamp.size(), &toyLifetime[0], &xsProdLimitToyMchamp[0]);
}
TGraph* DifferentXSLimitPlots::getLimitMchampTP(){
	return new TGraph(xsProdLimitTPMchamp.size(), &tpLifetime[0], &xsProdLimitTPMchamp[0]);
}

TGraph* DifferentXSLimitPlots::getExpLimitMchamp() {
	return new TGraph(xsProdExpLimitToyMchamp.size(), &toyLifetime[0], &xsProdExpLimitToyMchamp[0]);
}
TGraphAsymmErrors* DifferentXSLimitPlots::getExpLimitMchamp1Sig() {
	return new TGraphAsymmErrors(xsProdExpLimitToyMchamp.size(),
								&toyLifetime[0],
								&xsProdExpLimitToyMchamp[0],
								0,
								0,
								&xsProdExpLimitLo1SigToyMchamp[0],
								&xsProdExpLimitHi1SigToyMchamp[0]);
}
TGraphAsymmErrors* DifferentXSLimitPlots::getExpLimitMchamp2Sig() {
	return new TGraphAsymmErrors(xsProdExpLimitToyMchamp.size(),
								&toyLifetime[0],
								&xsProdExpLimitToyMchamp[0],
								0,
								0,
								&xsProdExpLimitLo2SigToyMchamp[0],
								&xsProdExpLimitHi2SigToyMchamp[0]);
}


TGraph* DifferentXSLimitPlots::getMassLimitGluino() {
	return new TGraph(xsProdMassGluino.size(), &massGluino[0], &xsProdMassGluino[0]);
}

TGraph* DifferentXSLimitPlots::getMassLimitGluinoTP() {
	return new TGraph(xsProdMassTPGluino.size(), &massGluino[0], &xsProdMassTPGluino[0]);	
}

TGraph* DifferentXSLimitPlots::getExpMassLimitGluino(){
	return new TGraph(xsProdExpMassGluino.size(), &massGluino[0], &xsProdExpMassGluino[0]);
}

TGraphAsymmErrors* DifferentXSLimitPlots::getExpMassLimitGluino1Sig(){
...
TGraphAsymmErrors* DifferentXSLimitPlots::getExpMassLimitGluino2Sig() {
	return new TGraphAsymmErrors(xsProdExpMassGluino.size(),
				     &massGluino[0],
				     &xsProdExpMassGluino[0],
				     0,
...
TGraphAsymmErrors* DifferentXSLimitPlots::getExpMassLimitStop2Sig() {
	return new TGraphAsymmErrors(xsProdExpMassStop.size(),
				     &massStop[0],
				     &xsProdExpMassStop[0],
				     0,
...
TGraph* DifferentXSLimitPlots::getMassLimitMchamp() {
	return new TGraph(xsProdMassMchamp.size(), &massMchamp[0], &xsProdMassMchamp[0]);
}

TGraph* DifferentXSLimitPlots::getMassLimitMchampTP() {
	return new TGraph(xsProdMassTPMchamp.size(), &massMchamp[0], &xsProdMassTPMchamp[0]);	
}

TGraph* DifferentXSLimitPlots::getExpMassLimitMchamp(){
	return new TGraph(xsProdExpMassMchamp.size(), &massMchamp[0], &xsProdExpMassMchamp[0]);
}

TGraphAsymmErrors* DifferentXSLimitPlots::getExpMassLimitMchamp1Sig(){
	return new TGraphAsymmErrors(xsProdExpMassMchamp.size(),
				     &massMchamp[0],
				     &xsProdExpMassMchamp[0],
				     0,
				     0,
				     &xsProdExpLo1SigMassMchamp[0],
				     &xsProdExpHi1SigMassMchamp[0]);
}

TGraphAsymmErrors* DifferentXSLimitPlots::getExpMassLimitMchamp2Sig() {
	return new TGraphAsymmErrors(xsProdExpMassMchamp.size(),
				     &massMchamp[0],
				     &xsProdExpMassMchamp[0],
				     0,
				     0,
				     &xsProdExpLo2SigMassMchamp[0],
				     &xsProdExpHi2SigMassMchamp[0]);
}

TGraph* DifferentXSLimitPlots::getGluinoTheory() {
	return new TGraph(gluinoTheoryMass.size(), &gluinoTheoryMass[0], &gluinoTheoryXS[0]);
}

TGraph* DifferentXSLimitPlots::getStopTheory() {
	return new TGraph(stopTheoryMass.size(), &stopTheoryMass[0], &stopTheoryXS[0]);	
}

TGraph* DifferentXSLimitPlots::getMchampTheory() {
	return new TGraph(mchampTheoryMass.size(), &mchampTheoryMass[0], &mchampTheoryXS[0]);
}
