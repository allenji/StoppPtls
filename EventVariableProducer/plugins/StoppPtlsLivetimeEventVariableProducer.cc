...
#include "StoppPtls/EventVariableProducer/plugins/StoppPtlsLivetimeEventVariableProducer.h"
#include "DataFormats/Math/interface/deltaR.h"

StoppPtlsLivetimeEventVariableProducer::StoppPtlsLivetimeEventVariableProducer(const edm::ParameterSet &cfg) :
...
StoppPtlsLivetimeEventVariableProducer::~StoppPtlsLivetimeEventVariableProducer()
{
}

void StoppPtlsLivetimeEventVariableProducer::AddVariables(const edm::Event & event) {
...
double StoppPtlsLivetimeEventVariableProducer::Eta(double x, double y, double z, double time) {
  TLorentzVector v = TLorentzVector(x, y, z, time);
  return v.PseudoRapidity();
}

double StoppPtlsLivetimeEventVariableProducer::Beta(const reco::GenParticle& mcpart){
  double beta = -999;
  if(mcpart.p()!=0.) beta = 1.0/(TMath::Sqrt((mcpart.mass()/mcpart.p())*(mcpart.mass()/mcpart.p())+1));
  return beta;
}

double StoppPtlsLivetimeEventVariableProducer::dRmatchedToStoppedParticle(const reco::GenParticle& mcpart, int nStoppedParticles,edm::Handle< std::vector<float> > xs, edm::Handle< std::vector<float> > ys, edm::Handle< std::vector<float> > zs, edm::Handle< std::vector<float> > times) {
...
DEFINE_FWK_MODULE(StoppPtlsLivetimeEventVariableProducer);
