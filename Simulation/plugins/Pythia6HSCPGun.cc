...
namespace {
  ...
  void writeCommon (std::ofstream& file, bool& isDelayedMuons) {
    char buffer[1024];
    ...
    file << buffer; // mchamps acting as doubly charged higgs
  ...
  void makeParticleTable (const std::string& fName, double fSparticleMass, double fNeutralinoMass, double fGravitinoMass, bool isDelayedMuons) {
    std::ofstream file (fName.c_str());
    writeCommon (file, isDelayedMuons);
    writeRHadron (file);
  ...
  mVt(0.)
{
  ...
  makeParticleTable (decayTable,sparticleMass, neutralinoMass, gravitinoMass, isDelayedMuons);
