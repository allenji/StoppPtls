...
void excludedRegionGluinoThreeBody_CWR_new() {
  ...
  double data[32]  = { 70.,     1393,     857,     46.2,    32.0,    92.4,   64.0,
  ...
  int expExcl70v = 1325;
  int expExcl70p1v = 1408;
  int expExcl70m1v = 1229;
  ...
  for (int i=0; i<1400; ++i) {
    expExcl70[i] = e_g[0] + sqrt(e_g[0]*e_g[0] + m_chi0[i]*m_chi0[i]) + 20;
    if (expExcl70[i]>expExcl70v) break;
    npoints_expExcl70++;
  }

  for (int i=0; i<1400; ++i) {
    expExcl70p1[i] = e_g[0] + sqrt(e_g[0]*e_g[0] + m_chi0[i]*m_chi0[i]) + 20;
    if (expExcl70p1[i]>expExcl70p1v) break;
    npoints_expExcl70p1++;
  }

  for (int i=0; i<1400; ++i) {
    expExcl70m1[i] = e_g[0] + sqrt(e_g[0]*e_g[0] + m_chi0[i]*m_chi0[i]) + 20;
    if (expExcl70m1[i]>expExcl70m1v) break;
    npoints_expExcl70m1++;
  }

  for (int i=0; i<1400; ++i) {
    obsExcl70[i] = e_g[0] + sqrt(e_g[0]*e_g[0] + m_chi0[i]*m_chi0[i]) + 20;
  ...
  */	
  ...
  for (int i = 1; i <= 400; i++){
    for (int j = 1; j <= 400; j++) {
      ...
      double lowLimit = e_g[0] + sqrt(e_g[0]*e_g[0] + chi0Mass*chi0Mass)+20; 
      //double xsec = gluino_xs.mass2xsec(200+ 6*j);
      //cout << xsec << endl;

      int massPoint = gluinoMass / 200;
      double gluinoMassRef = 1393;
      ...
      h3.SetBinContent(i, j, 1000*0.026*effRef/eff);
      }
    }
  }
  h3.GetZaxis()->SetTitle("95% CL upper limit on #sigma(pp #rightarrow #tilde{g}#tilde{g}) #bf{#it{#Beta}}(#tilde{g} #rightarrow q#bar{q}#tilde{#chi}^{0}) [fb]");
  ...
  dislabel.SetTextAngle(40);
  dislabel.Draw();
  

  //	TPaveText blurb3(0.48, 0.15, 0.6, 0.37, "NDC");
  TPaveText blurb3(0.16, 0.50, 0.72, 0.68, "NDC");
  ...
  blurb3.AddText("#tilde{g} #rightarrow q#bar{q}#tilde{#chi}^{0}");
  blurb3.AddText("m_{#tilde{g}} - m_{#tilde{#chi}^{0}}    160 GeV");
  ...
  TLegend leg3(0.18, 0.68, 0.50, 0.82,"","NDC");
...
public:
  ...
  for (int i = 1; i <= 200; i++){
    for (int j = 1; j <= 200; j++) {
      ...
      double eff = gluinoStopEff[massPoint - 1] + (gluinoStopEff[massPoint] - gluinoStopEff[massPoint - 1]) * (gluinoMass - 200*massPoint)/ 200;
      if (gluinoMass > lowLimit) {
      h4.SetBinContent(i, j, (10.66/6200)/(eff*0.50));
      }
    }
  }
...
TPaveText blurb4(0.47, 0.20, 0.72, 0.37, "NDC");
