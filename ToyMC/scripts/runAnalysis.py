...
# Get runAll.py script from its location in StoppPtls/ToyMC
sys.path.append(os.path.join(os.environ['CMSSW_BASE'],"src","StoppPtls","ToyMC","scripts"))
try:
    import runAll
except:
    print "ERROR:  Unable to import runAll.py"
    print "Have you remembered to check out StoppPtls/ToyMC from CVS?"
    sys.exit()


# Add path with BuildToyMCSummary
# Get BuildToyMCSummary code
sys.path.append(os.path.join(os.environ['CMSSW_BASE'],"src","StoppPtls","Analysis","scripts"))
try:
...
sys.path.append(os.path.join(os.environ['CMSSW_BASE'],"src","StoppPtls","Analysis","python"))
#try:
#    import printSummary
#except SyntaxError:
