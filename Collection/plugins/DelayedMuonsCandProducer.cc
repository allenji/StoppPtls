...
void DelayedMuonsCandProducer::doDisplacedStandAloneMuons(edm::Event& iEvent, const edm::EventSetup& iSetup)
{
  ...
  if (displacedStandAloneMuons.isValid()) {
	...
	//std::cout<<"filled track rpc Hit variables"<<std::endl;
      ...
      track.set_rpcHitBxPattern(Rpc_Bx_Pattern(rpcHitBx_));
      track.set_rpcHitBxAverage(Rpc_Bx_Average(rpcHitBx_));
      track.set_nRpcTimingHits(rpcHitBx_.size());
      //std::cout<<"rpc bx pattern is: "<<Rpc_Bx_Pattern(rpcHitBx_)<<std::endl;
      //std::cout<<"rpc bx average is: "<<Rpc_Bx_Average(rpcHitBx_)<<std::endl;
...
int DelayedMuonsCandProducer::Rpc_Bx_Pattern(std::vector<int> &rpcHitBx_){
  ...
  //loop over Rpc hits
  for(unsigned int j = 0; j != rpcHitBx_.size(); ++j) {
    ...
    if(rpcHitBx_.at(j) != 0) all_bx_0 = false;
    if(rpcHitBx_.at(j) <= 0) all_bx_positive = false;
    if(rpcHitBx_.at(j) >= 0) all_bx_negative = false;
    if(rpcHitBx_.at(j) < 0) all_bx_positiveOr0 = false;
    if(rpcHitBx_.at(j) > 0) all_bx_negativeOr0 = false;
    unsigned int k=j+1;
    if(k<rpcHitBx_.size()){
      if(rpcHitBx_.at(k) != rpcHitBx_.at(j)) all_bx_constant = false;
      if(rpcHitBx_.at(k) < rpcHitBx_.at(j)) all_bx_increasing = false;
      if(rpcHitBx_.at(k) > rpcHitBx_.at(j)) all_bx_decreasing = false;
    }
  }
  if(all_bx_constant) {
    all_bx_increasing = false;
...
double DelayedMuonsCandProducer::Rpc_Bx_Average(std::vector<int> &rpcHitBx_){
  double rpc_bx_average = 0.;

 //loop over Rpc hits 
  for(unsigned int j = 0; j != rpcHitBx_.size(); ++j) {
    rpc_bx_average += rpcHitBx_.at(j);
  }

  if(rpcHitBx_.size()!=0) rpc_bx_average = 1.0*rpc_bx_average/rpcHitBx_.size();
